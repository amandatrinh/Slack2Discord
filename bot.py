...
def get_user(team_id, user_id, token):
    ...
    if user_id not in users:
        ...
        users = workspace.find_one_and_update({'_id': team_id}, {'$set': {f'users.{user_id}': r.json()["profile"]["real_name"]}}, return_document=pymongo.collection.ReturnDocument.AFTER)['users']
...
def get_channel(team_id ,channel_id, token):
    ...
    if channel_id not in channels:
        ...
        channels = workspace.find_one_and_update({'_id': team_id}, {'$set': {f'channels.{channel_id}': r.json()["channel"]["name"]}}, return_document=pymongo.collection.ReturnDocument.AFTER)['channels']
